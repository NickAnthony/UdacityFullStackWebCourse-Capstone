...
function ActorProfile() {
  ...
  return (
    <div className="Actor-profile-wrapper">
      <div className="Actor-profile-body">

        <div className="Actor-profile">
          <img src={actor.portrait_url} alt={actor.name + " Portrait"} className="Actor-profile-portrait" />
          <div className="Actor-profile-content">
            <p className="Actor-profile-name">{actor.name}</p>
            <h2>{actor.gender}, {actor.age}</h2>
            {
              (isAuthenticated) &&
                  <div className="Actor-profile-menu">
                    <h3>Edit actor menu:</h3>
                    <button className="Actor-profile-menu-button"  onClick={() => associateActor(id)}>Add actor to movie</button>
                    <button className="Actor-profile-menu-button"  onClick={() => {}}>Update actor/actress information</button>
                    <button className="Actor-profile-menu-button"  onClick={() => {}}>Delete actor</button>
                  </div>
